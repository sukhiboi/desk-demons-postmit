...
class App {
  ...
  async getSearchedUserProfile(userId, username) {
    try {
      ...
      const userDetails = await this.getUserProfile(id.user_id);
      ...
      userDetails.isFollower = await this.dbClient.isFollower(userId, id);
      return userDetails;
    } catch (err) {
      return { errMsg: 'Invalid userId', posts: [], likedPosts: [] };
    }
  ...
  async follow(username, userId) {
    const followerId = await this.dbClient.getUserIdByUsername(username);
    return await this.dbClient.addFollower(userId, followerId);
  }

  async unfollow(username, userId) {
    const followerId = await this.dbClient.getUserIdByUsername(username);
    return await this.dbClient.removeFollower(userId, followerId);
  }
