...
class App {
  ...
  async getAllPosts(userId) {
    try {
      ...
      posts = posts.sort((a,b)=>new Date(b.posted_at)-new Date(a.posted_at));
  ...
  async getFollowingList(username, userId) {
    ...
    followingList = await followingList.map(async user => {
      ...
      following.isFollowingMe = true;
      return following;
    });
    followingList = await Promise.all(followingList);
    const profile = await this.dbClient.getUserDetails(userId);
