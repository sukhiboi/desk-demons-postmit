...
class App {
  ...
  async updatePost(userId, posts) {
    ...
    const updatedPosts = posts.map(async post => {
      ...
      post.responseCount = responses.length;
      ...
      post.isBookmarked = await this.isBookmarked(post.postId);
      return { ...user, ...post };
    });
  ...
  async getPostDetails(postId) {
    ...
    return { post: postDetails, loggedUser: this.username, responses };
  }
  ...
  async savePost(content) {
    ...
    return await this.saveHashTag(content, postId);
  }
  ...
  async getSearchSuggestions(searchInput) {
    ...
    if (isHashTag) {
      const searchHashtag = searchInput.slice(1);
      const hashtags = await this.datastore.getMatchingHashtags(searchHashtag);
