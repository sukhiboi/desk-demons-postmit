...
class App {
  ...
  async getPostDetails(userId, post) {
    ...
    post.isLiked = await this.dbClient
      ...
      .catch(err => Promise.reject(err));
    post.isDeletable = post.user_id === userId;
  ...
  async getAllPosts(userId) {
    try {
      ...
      let posts = following.map(follower =>
        this.dbClient.getPostsByUserId(follower.follower_id)
      );
      ...
      posts = posts.sort(
        (post1, post2) => new Date(post2.posted_at) - new Date(post1.posted_at)
      );
  ...
  async getUserProfile(userId) {
    try {
      ...
      userProfile.joined_date = formatDate(userProfile.joined_date);
  ...
  async getFollowingList(username, userId) {
    const user = await this.dbClient.getUserIdByUsername(username);
    let followingList = await this.dbClient.getFollowings(user.user_id);
    followingList = await followingList.map(async user => {
      const following = await this.dbClient.getUserDetails(user.follower_id);
    ...
    const profile = await this.dbClient.getUserDetails(user.user_id);
  ...
  async getFollowersList(username, userId) {
    const user = await this.dbClient.getUserIdByUsername(username);
    let followersList = await this.dbClient.getFollowers(user.user_id);
    followersList = await followersList.map(async user => {
      const follower = await this.dbClient.getUserDetails(user.user_id);
